...
#ifdef TEST_DEBUG
    if (op_keys[op_i] >= hot_key_lower_bound &&
        op_keys[op_i] <= hot_key_upper_bound) {
      ++hot;
    }
    ++counter;
#endif
...
int main(int argc, char **argv) {
  ...
  COUT_THIS("skewed keys size: %d" << skewed_keys.size());
  ...
  if (read_ratio < 1) {
    ...
    generate_skew_workload(hot_data_start, hot_data_ratio, hot_query_ratio,
                           non_exist_keys, skewed_non_exist_keys);
  }
