...
basic_local_memory_usage=86
short_range_local_memory_usage=130
read_latest_local_memory_usage=250
art_basic_local_memory_usage=110
art_read_latest_local_memory_usage=263


for target in art masstree
do
    ...
    do
        ...
        if [ ! -f $output_dir/${target}-update-heavy-${fgn}-${size}-${cxl_percentage}.txt ];then
            ...
            ./$builddir/${proc} \
                ...
                --max-local-memory-usage $local_memory_usage > $output_dir/${target}-update-heavy-${fgn}-${size}-${cxl_percentage}.txt
        fi
        ...
        if [ ! -f $output_dir/${target}-read-mostly-${fgn}-${size}-${cxl_percentage}.txt ];then
            ...
            ./$builddir/${proc} \
                ...
                --bg $bgn \
                --table-size $size --cxl-percentage $cxl_percentage \
                --max-local-memory-usage $local_memory_usage > $output_dir/${target}-read-mostly-${fgn}-${size}-${cxl_percentage}.txt
        fi
        ...
        if [ ! -f $output_dir/${target}-read-only-${fgn}-${size}-${cxl_percentage}.txt ];then
            ...
            ./$builddir/${proc} \
                ...
                --bg $bgn \
                --table-size $size --cxl-percentage $cxl_percentage \
                --max-local-memory-usage $local_memory_usage > $output_dir/${target}-read-only-${fgn}-${size}-${cxl_percentage}.txt
        fi
        ...
        if [ ! -f $output_dir/${target}-read-latest-${fgn}-${size}-${cxl_percentage}.txt ];then
            ...
            if [ "$target" == "art" ]; then
                local_memory_usage=$art_read_latest_local_memory_usage
            else
                local_memory_usage=$read_latest_local_memory_usage
            fi
            if [ "$is_latency" -eq 1 ]; then
                proc="read_latest_lat"
            else 
                proc="read_latest_origin"
            fi
            ./$builddir/${proc} \
                --target $target \
                --runtime 30 \
                ...
                --bg $bgn \
                --table-size $size --cxl-percentage $cxl_percentage \
                --max-local-memory-usage $local_memory_usage > $output_dir/${target}-read-latest-${fgn}-${size}-${cxl_percentage}.txt
        fi
        ...
        if [ "$target" != "art" ]; then
            if [ ! -f $output_dir/${target}-short-ranges-${fgn}-${size}-${cxl_percentage}.txt ];then
                ...
                ./$builddir/${proc} \
                    ...
                    --bg $bgn \
                    --table-size $size --cxl-percentage $cxl_percentage \
                    --max-local-memory-usage $local_memory_usage > $output_dir/${target}-short-ranges-${fgn}-${size}-${cxl_percentage}.txt
            fi
        fi
        ...
        if [ ! -f $output_dir/${target}-read-modify-write-${fgn}-${size}-${cxl_percentage}.txt ];then
            ...
            ./$builddir/${proc} \
                ...
                --bg $bgn \
                --table-size $size --cxl-percentage $cxl_percentage \
                --max-local-memory-usage $local_memory_usage > $output_dir/${target}-read-modify-write-${fgn}-${size}-${cxl_percentage}.txt
        fi
    done
done
